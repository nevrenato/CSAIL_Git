...
\begin{frame}
	...
	\begin{block}{Shine some light into Git internals}
		\begin{itemize}
			...
			does (not) guarantee
			\item Help others understanding Git, building a manual with public
         access
		\end{itemize}
	\end{block}
\end{frame}
...
\frametitle{Branch and HEAD}
   \begin{block}{Branch}
      \begin{itemize}
         \item Pointer to a commit - Unlike other VCS, where a branch is a full copy of the repository
      \end{itemize}
   \end{block}
   \begin{block}{HEAD}
      \begin{itemize} 
