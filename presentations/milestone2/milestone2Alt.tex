...
\begin{frame}
	all f : index.s | f.path -> f.blob in ((CA.univ <: IA) - CA) and f.path
	in CB.univ => (f.path -> f.blob in CB or (f.path).CA = (f.path).CB)

	For all newly modified files, if their paths are also on CB then, or the
	new changes are already on CB or the old content it's equal on both
	commits
\end{frame}

\begin{frame}
	Probably a bug encountered...
\end{frame}
...
\begin{frame}
	the property of commitaddcommitrmcommit
	add remove inverse operation
	checkout idempotence
	checkout no lost files ?
	commit invariant
